...
function MenuManager:new(o)
  ...
  self.menus = {}
  self.menu_order = {"root"}
  self.input_order = {"root"}
  self.menu_queue = {}
  self.multi_touch_activation_count = 0
  self.touches_down = {}
  return o
end
...
function MenuManager:setRoot(root, menu)
  ...
  if root ~= nil then
    ...
    if gamelua.useDynamicAssets then
      _G.assert(_G.type(self.menus[menu].name) == "string", "invalid name value")
      gamelua.dynamic.load(self.menus[menu].name)
    end
...
function MenuManager:exitCurrentRoot(menu)
  ...
  if self.menus[menu] ~= nil then
    ...
    if gamelua.useDynamicAssets then
      _G.assert(_G.type(self.menus[menu].name) == "string", "invalid name value")
      gamelua.dynamic.delayrelease(self.menus[menu].name)
    end
    self.menus[menu] = nil
  end
end
