...
function activateSuperSeed()
  if g_powerups_active or contains(g_powerups_used, "superseed") then
    return false
  end
  local bird_name = getNextBird(currentBirdIndex)
  if not bird_name then
    return false
  end
...
function activateKingSling()
  if g_powerups_active or contains(g_powerups_used, "kingsling") then
    return false
  end
  local bird_name = getNextBird(currentBirdIndex)
  if not bird_name then
...
function activateSlingScope()
  if g_powerups_active or contains(g_powerups_used, "slingscope") then
    return false
  end
  local bird_name = getNextBird(currentBirdIndex)
  if not bird_name then
...
function activateBirdQuake()
  if g_powerups_active or contains(g_powerups_used, "birdquake") then
    return false
  end
  eventManager:notify({
    id = events.EID_POWERUP_WILL_BE_USED,
...
function getLaunchVelocity()
  ...
  local force = defaultForce * physicsScale * bird.mass * g_slingshot_power
...
function powerupReward(amount, powerup)
  ...
  settingsWrapper:addItems(powerup, amount)
...
function bundleReward(amount, initial_shade)
  ...
  if not settingsWrapper:isChildlockOn() then
    settingsWrapper:addItems("slingscope", amount)
    settingsWrapper:addItems("birdquake", amount)
    settingsWrapper:addItems("kingsling", amount)
    settingsWrapper:addItems("superseed", amount)
    local scale = 1
