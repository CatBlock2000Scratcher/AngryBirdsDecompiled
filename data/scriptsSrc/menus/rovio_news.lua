print = gamelua.print
ui = gamelua.ui
local g_rovio_news, g_rovio_news_frame
g_rovio_news_loaded = false
local test_rovio_news = false
RovioNewsTest = ui.Frame:new()
...
function RovioNews:init()
  ...
  local x = (x1 + x2) * multiplier
  local y = 0
  local width = gamelua.screenWidth * multiplier - x
  local height = gamelua.screenHeight * multiplier
  print("<rovio news> webview x=" .. x .. " y=" .. y .. " w=" .. width .. " h=" .. height .. "\n")
  if g_rovio_news == nil and _G.WebView then
    print("<rovio news> begin loading, url=" .. _G.tostring(gamelua.ROVIO_NEWS_URL) .. "\n")
    g_rovio_news = _G.WebView.new(x, y, height, width)
    ...
    g_rovio_news:loadPage(gamelua.ROVIO_NEWS_URL)
  else
