...
function MightyEagleButton:init()
  Frame.init(self)
  self:setImage("INGAME_BUTTON_EAGLE", "BUTTON_USE_EAGLE_DISABLED")
...
function MightyEagleButton:onEntry()
  if not gamelua.startedFromEditor then
    ...
    gamelua.eventManager:addEventListener(gamelua.events.EID_MIGHTY_EAGLE_NFC_COOLDOWN_OVER, self)
    ...
    self:setEnabled(gamelua.getMightyEagleNFCUnlockCooldown() == 0 and (not gamelua.isEagleUnavailableForShot() or not first_time))
...
function MightyEagleButton:eventTriggered(event)
  if event.id == gamelua.events.EID_MIGHTY_EAGLE_AVAILABLE or event.id == gamelua.events.EID_MIGHTY_EAGLE_NFC_COOLDOWN_OVER then
...
function MightyEagleButton:onExit()
  if not gamelua.startedFromEditor then
    ...
    gamelua.eventManager:removeEventListener(gamelua.events.EID_MIGHTY_EAGLE_NFC_COOLDOWN_OVER, self)
