...
function DailyReward:layout()
  ...
  if deviceModel == "android" and isIapEnabled() then
...
function DailyReward:update(dt, time)
  ...
  if self.state == "MOVE_UP" then
    ...
    if self.movein_offset <= 0 then
      ...
      settingsWrapper:addItems(self.winning_object, self.streak)
      eventManager:notify({
