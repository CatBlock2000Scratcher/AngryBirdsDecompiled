...
ui = gamelua.ui
PauseButton = gamelua.ui.ImageButton:new()
...
PowerupButton = ui.ImageButton:new()
...
function GameHud:init()
  ...
  if gamelua.g_powerups_enabled then
...
function GameHud:layout()
  ...
  if gamelua.isIapEnabled() and eagleButton ~= nil then
    eagleButton:refresh()
  end
  if g_showRestartButton then
...
function GameHud:update(dt, time)
  ...
  if gamelua.cheatsEnabled and gamelua.keyCombination("D", "R") then
...
function GameHud:draw(x, y, scaleX, scaleY, angle)
  ...
  if not isEagleBaitLaunched and not gamelua.g_special_game_mode then
    ...
    if self.highscore then
      ...
      _G.res.drawString("TEXTS_BASIC", scoreString, xMargin, yAdd * 3 + yMargin, "TOP", "RIGHT")
    else
      _G.res.drawString("TEXTS_BASIC", scoreStr, xMargin, yMargin, "TOP", "RIGHT")
      _G.res.drawString("TEXTS_BASIC", scoreString, xMargin, yAdd + yMargin, "TOP", "RIGHT")
    end
