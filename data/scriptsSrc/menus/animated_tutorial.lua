...
AnimatedTutorial = BackgroundBox:new()
...
function AnimatedTutorial:layout()
  ...
  self.width = _G.math.floor(max_w)
  self.height = _G.math.floor(max_h)
...
if not g_isAccelerometerControl then
  function AnimatedTutorial:onPointerEvent(eventType, x, y)
    ...
    if eventType == "LRELEASE" and self.tutorial_id == "TUTORIAL_5" and self.time >= 2.7 and not settingsWrapper:isGoldenEggUnlocked("LevelGE_4") and g_tutorialActive and g_tutorialActive.from == "PAUSE_PAGE" and x >= self.x + self.width * -0.5 and x <= self.x + self.width * 0.5 and y >= self.y + self.height * -0.5 and y <= self.y + self.height * 0.5 then
      eventManager:notify({
  ...
  while self.tutorial.events[self.cursor] and self.time >= self.tutorial.events[self.cursor].time do
    ...
    if event.id then
      if event.id == "ge1" then
        if not settingsWrapper:isGoldenEggUnlocked("LevelGE_4") and g_tutorialActive and g_tutorialActive.from == "PAUSE_PAGE" then
