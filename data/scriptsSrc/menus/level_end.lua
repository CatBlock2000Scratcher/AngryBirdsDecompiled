...
function EpisodeThreeStars:layout()
  ...
  reward_sprite.x = screenWidth * 0.5
  reward_sprite.y = screenHeight * 0.3
  local ok_button = self:getChild("button3")
  local reward_sprite_bottom = reward_sprite.y - reward_sprite.py + reward_sprite.h
  local ok_button_top = ok_button.y - ok_button.h / 2
  local text = self:getChild("text")
  text.x = screenWidth * 0.5
  text.y = (reward_sprite_bottom + ok_button_top) / 2
  text.text = "TEXT_PERFECT"
  text.textBoxSize = self.center_width
  local scale = 0
  repeat
    ...
    text:clip()
    scale = scale + 1
  until 5 >= #text.lines and text:getHeight() < ok_button_top - reward_sprite_bottom
end
