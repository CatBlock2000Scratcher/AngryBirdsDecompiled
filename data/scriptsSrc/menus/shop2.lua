...
ProductButton = ui.ImageButton:new()
...
function ProductPage:init()
  ...
  local powerups_per_product = #g_shop_items / #g_powerup_types
  local powerup_index = find_key(g_powerup_types, self.current_product)
  self.current_item = self.current_item or powerup_index * powerups_per_product - (powerups_per_product - 1)
  ...
  for k, v in _G.ipairs(g_shop_items) do
    local item_button = ui.ImageButton:new()
    item_button.name = "item" .. k
    local items_per_product = #g_shop_items / #g_powerup_types
    item_button:setImage("SHOP_MIDDLEWINDOW_" .. (k - 1) % items_per_product + 1)
    ...
    if deviceModel ~= "android" then
      ...
      price.text = formatCurrency(v.price.usd, "USD")
      price.hanchor = "LEFT"
...
function ProductPage:layout()
  ...
  for i = 1, #g_shop_items do
    ...
    window_image.y = _G.math.max(label.y, -sh / 2 + window_image.h * 0.53)
...
function ProductPage:onPointerEvent(eventType, x, y)
  ...
  elseif result == "BUY_PRODUCT" then
    ...
    if deviceModel == "android" and g_shop_items[meta].price.usd >= 10 then
 ...
 Do you wish to proceed?]],
  ...
  elseif result == "GOTO_PRODUCT" then
    local scroll = self:getChild("scroll")
    local items_per_product = #g_shop_items / #g_powerup_types
    scroll:setTargetAnchor(meta * items_per_product - (items_per_product - 1))
...
function PowerupTutorial:exit()
  local items_per_product = #g_shop_items / #g_powerup_types
  local new_item_index = (self.product_index - 1) * items_per_product + (self.item - 1) % items_per_product + 1
  eventManager:notify({
    id = events.EID_CHANGE_SCENE,
    target = ProductPage:new({
      from = self.from,
      current_product = self.product,
      current_item = new_item_index
    })
  })
end
