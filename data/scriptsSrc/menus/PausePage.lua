...
local enableRovioNews = gamelua.webViewIsSupported and not gamelua.isKidsVersion
function PausePage.getTotalW()
  local w, h = _G.res.getSpriteBounds("BUTTON_RESUME")
  local sw, sh = gamelua.screenWidth, gamelua.screenHeight
...
function PausePage:init()
  ...
  if gamelua.g_powerups_enabled and gamelua.showPowerupPurchaseElements() then
  ...
  if not gamelua.isKidsVersion then
...
function PausePage:layout()
  ...
  local display_shop = gamelua.g_powerups_enabled and gamelua.showPowerupPurchaseElements() and self:getChild("shop_button") ~= nil
  self.dummyFrame.x = 0
  self.dummyFrame.y = 0
